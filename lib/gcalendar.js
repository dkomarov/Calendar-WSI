...
function gCal(functionName) {
  ...
  function listEvents(auth) {
    ...
    calendar.events.list({
      ...
      if (events.length) {
        events.map((event, i) => {
          console.log("Event", i, "is", event.summary)
        });
      }
    });
  }
  ...
  function insertEvents(auth) {
    ...
    calendar.events.insert({
      ...
      calendar.events.list({
        ...
        if (events.length) {
          events.map((event, i) => {
            Event.findOne({ event_id: event.id })
                 .then((currentEvent) => {
                  if(currentEvent){
                      /** User exists */
                    ...
                    start: myEvent.start.dateTime,
                    end: myEvent.end.dateTime,
                    recurrence: myEvent.recurrence,
                    attendees: myEvent.attendees,
                    reminders: myEvent.reminders
                    });
  ...
  function deleteEvent(auth) {
    ...
    calendar.events.delete(params, async (res) => {
      if (res) {
